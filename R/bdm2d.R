...
bdm2d <- function(mat, block_size, offset) {
    ...
    if (block_size == 4) {
        bdm <- (sum(four_by_four_ctm[rownames(squares_tally), ]) + sum(log2(squares_tally$Freq)))
    } else {
        bdm <- (sum(three_by_three_ctm[rownames(squares_tally), ]) + sum(log2(squares_tally$Freq)))
    }
